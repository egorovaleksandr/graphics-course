...
obj sdf(in vec3 p) {
    obj sph1 = obj(sphere ( p, vec3 ( 0.0, 3.4, 0.0 ), 1.0 * (1.0 + sin(params.iTime + PI * 0.75))) + 0.05 * (1.0 + sin(10.0 *(length(p) + 2.0*params.iTime))), 0);
    obj sph2 = obj(sphere ( p, vec3 ( 3.4, 0.0, 0.0 ), 1.0 * (1.0 + sin(params.iTime))) + 0.05 * (1.0 + sin(10.0 *(length(p) + 2.0*params.iTime))), 0);
    obj sph3 = obj(sphere ( p, vec3 ( 0.0, 0.0, 3.4 ), 1.0 * (1.0 + sin(params.iTime))) + 0.05 * (1.0 + sin(10.0 *(length(p) + 2.0*params.iTime))), 0);
    obj sph4 = obj(sphere ( p, vec3 ( 0.0, -3.4, 0.0 ), 1.0 * (1.0 + sin(params.iTime + PI * 0.75))) + 0.05 * (1.0 + sin(10.0 *(length(p) + 2.0*params.iTime))), 0);
    obj sph5 = obj(sphere ( p, vec3 ( -3.4, 0.0, 0.0 ), 1.0 * (1.0 + sin(params.iTime))) + 0.05 * (1.0 + sin(10.0 *(length(p) + 2.0*params.iTime))), 0);
    obj sph6 = obj(sphere ( p, vec3 ( 0.0, 0.0, -3.4 ), 1.0 * (1.0 + sin(params.iTime))) + 0.05 * (1.0 + sin(10.0 *(length(p) + 2.0*params.iTime))), 0);
    
    obj sph_0 = objmin(objmin( sph5, sph2) , objmin( sph3, sph6));
    
    obj sph  = obj(sphere ( p, vec3 ( 0.0, 0.0, 0.0 ), 4.0 ) +  0.025 * cos(2.0 * p.x * p.y * p.z + params.iTime * 3.0), 0);
    obj pl   = obj(plane(p, vec4 (0.0, 1.0, 0.0, 5.0)) + 0.5 * sin(length(p) - params.iTime), 1);
    
    obj tor  = obj(torus(p, vec2 (4.0 * (1.0 + sin(1.0 * params.iTime)), 1)) + 0.01 * (1.0 + sin(15.0 * p.y + 15.0 * p.x + p.z * 15.0 + params.iTime*5.0)), 2);
...
void main() {
    ...
    vec2 uv_coord = -(fragCoord - 0.5 * params.iResolution.xy) / max(params.iResolution.x, params.iResolution.y);
    vec2 mouse = vec2(params.iResolution.x - params.iMouse.x, params.iResolution.y - params.iMouse.y) / params.iResolution.xy;
    ...
    if (hit) {
        ...
        if (id == 0) {
            ...
            fragColor = vec4 ( sin(intersectionPoint.x * 4.0), 0.5 + 0.5  * sin(length(intersectionPoint) - params.iTime), sin(intersectionPoint.z * 4.0), 0.0 ) * max(dot(surfaceNormal, lightVector), 0.0) * 0.85 + 0.15 + specular * vec4(0.5 + 0.5 * sin(params.iTime), 0.0,  0.33 + 0.33 * cos(params.iTime), 0.0);
            fragColor *= vec4(txtr, 0.0);
        }
    } else {
